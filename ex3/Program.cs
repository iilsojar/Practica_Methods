...
{
    int[] array = new int[size]; //int[] array - как результат - см строчку 15
    Random rnd = new Random();

    for (int i = 0; i < size; i++)
    {
        array[i] = rnd.Next(min, max + 1); //+ 1 потому что в методе Next последняя цифра не входит
        Console.Write($"{array[i]} "); 
    }
    return array; //вот и вывод результата
}
...
// int[] arr = CreatArrayRandom(12, -9, 9); //вызов метода
// PrintArray(arr);
...
int[] array = CreatArrayRandom(12, -9, 9);
int SummPositive = default;
int SummNegative = default;
foreach (int elem in array)
{
    if(elem >0) SummPositive = SummPositive + elem;
    else SummNegative = SummNegative + elem;
}
Console.WriteLine();
Console.WriteLine($"сумма положительных элементов = {SummPositive}, сумма отрицательных элементов = {SummNegative}");
